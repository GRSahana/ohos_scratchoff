...
public class ScratchoffThresholdProcessor implements ScratchPathPointsAggregator, Runnable {
    ...
    private static final Color MARKER_SCRATCHED = Color.RED;
    ...
    public void prepare(int[] layoutSize) {
        synchronized (pathManager) {
            ...
            opts.size = new Size((int) width,(int) height);
            ...
            this.canvas.drawColor(MARKER_UNTOUCHED, Canvas.PorterDuffMode.CLEAR);
    ...
    protected void processScratchedImagePercent() {
        Delegate delegate = this.delegate.get();
        PixelMap currentBitmap = this.currentBitmap;
